...
4. At the phase's exit, load its `phase-close` section afresh. That section owns
   close packet: the complete current readback followed by semantic reconciliation
   and the visible parent completion message in the same ordered artifact. Read
   and execute the full packet; an earlier Read is not this close.
...
reload `phase-close` and resume its first incomplete step in the bound close packet;
regenerate and reread the full packet if the implementation or accepted decisions changed:
- If a verified phase lacks its announcement, execute the packet's publication before advancing.
