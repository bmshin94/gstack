...
2. Complete the phase's required preliminary work (CEO: all Step 0, including its
   Spec Review Loop), then create the fresh snapshot and dispatch its nativeDispatchPrompt unchanged.
3. Consume native completion, then enabled outside results; only then complete the
   phase's remaining primary review sections.
