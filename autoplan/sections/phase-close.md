...
6. **Publish the phase report.** If any prerequisite in steps
   1–5 is incomplete, keep this phase open: repair, export and finish the full
   readback/verification before publishing. Otherwise, send the filled report
   below now as visible parent assistant text. Use this phase's row above and
   completed review data: actual findings, voice statuses and next step.
   Saving it in ACTIVE_PLAN or printing it through Bash does not publish it.
...
7. **Continue to the next step.** After the report message has been sent,
   Read/create/dispatch the next step in this phase's row. Continue in the same turn;
   do not end the turn to wait for a “continue” reply. After Eng, the next step is
   final synthesis/approval. The driver emits its declared skip message for each
   inapplicable Design or DX phase; a skip is never a completion.
