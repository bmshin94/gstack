...
Claude CEO subagent below and consume its final review, then run Codex. Present
the results under their respective headers before constructing the consensus.
...
- Dual voices: run BOTH Claude subagent AND Codex if available (P6), in that order.
  ...
  **Claude CEO subagent** (via Agent tool):
  Present its completed output under CLAUDE SUBAGENT (CEO — strategic independence).
  ...
  **Codex CEO voice** (via Bash):
  Present its completed output under CODEX SAYS (CEO — strategy challenge).
  ```bash
