...
every gap, explain why it matters, and resolve each proposed fix with the user
before editing.
...
3. AskUserQuestion about one proposed fix and wait for approval
4. Edit only that approved decision; it does not authorize other amendments
5. Re-rate with the reason for the change; document declined fixes and their lower score
6. Repeat for remaining gaps, or stop when the user says "good enough, move on"
