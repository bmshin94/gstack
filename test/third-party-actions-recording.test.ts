...
const negatives = {
  ...
  'tpa-absent-linux': ["A) gstack's own visible browser B) manual; download it at aside.com", "A) gstack's own visible browser B) manual; in your Aside browser", "gstack's own visible browser; manual", "A) gstack's own visible browser", 'A) manual instructions'],
  'tpa-broken': ["A) Open the Aside app, then I will recheck and drive it in your Aside browser", 'No current question.'],
  'tpa-absent-darwin': ['Aside for macOS 15+', 'Download it at aside.com', 'Download it at aside.com (macOS 15+), then in your Aside browser'],
  ...
  for (const [name, positive] of Object.entries(positives)) {
    for (const [kind, output] of [['pass', positive], ['exit', positive], ['runner', positive], ['fixture', positive], ...negatives[name].map(value => ['assertion', value])]) {
      ...
      if (kind === 'runner' || kind === 'fixture') {
        ...
        expect(observed.entry).toMatchObject({ exit_reason: 'harness_error', cost_usd: 0, model: 'claude-sonnet-4-6' });
...
`);
  try {
    const proc = Bun.spawn([process.execPath, 'test', script], {
      cwd: ROOT,
      env: { ...process.env, EVALS: '', EVALS_ALL: '', EVALS_PREFLIGHT_OK: '1',
        EVALS_MODEL: 'claude-sonnet-4-6', GSTACK_CLAUDE_CLI_VERSION: 'free TPA fixture',
