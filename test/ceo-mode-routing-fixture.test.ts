...
test.each(['success', 'next-modal', 'mode-submit', 'launch', 'navigation', 'posture', 'close'])('native mode fixture delivery and cleanup: %s', scenario => {
...
mock.module(path.join(root, 'test/helpers/claude-pty-runner.ts'), () => ({
  launchClaudePty: async opts => {
    current = {cwd:opts.cwd, options:opts, sends:[], closed:false, reads:[], selected:false, continued:false, submitted:false};
    facts.push(current);
...
mock.module(path.join(root,'test/helpers/ceo-mode-option.ts'),()=>({
  ...
  hasNativePostAnswerCeoPosture:(_transcript,target,_pattern,selectedAt)=>{
    ...
    return scenario!=='posture' && (scenario!=='next-modal'||current.continued) && (scenario!=='mode-submit'||current.submitted);
  },
...
`);
  try {
    ...
    expect(child.status, child.stderr).toBe(['success', 'next-modal', 'mode-submit'].includes(scenario) ? 0 : 1);
    ...
    else if (!['success','next-modal','mode-submit'].includes(scenario)) expect(child.stderr).toContain('fixture ' + scenario + ' failed');
