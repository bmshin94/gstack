...
describe('plan-mode-info resolver (handshake-replacement)', () => {
  ...
  test('0C-bis authority and fresh-approval paths precede mode selection', () => {
    ...
    expect(approach).toContain('Record any approach already chosen');
    expect(approach).toContain('an applicable instruction or earlier answer');
    expect(approach).toContain('with its source and exact scope. Do not ask again about that choice');
    ...
    expect(gate).toContain('Ask about one row per call; record its exact answer and approved scope before the next row');
