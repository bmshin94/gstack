...
async function exercise(mode: 'success' | 'max-turns' | 'first-timeout' | 'second-timeout' | 'empty-summary' | 'write-failure') {
  ...
  const declared: string[] = [], dirs: string[] = [], rows: Array<{ passed: boolean }> = [];
  ...
  const args = {
    ...
    CAPTURE_MS, CAPTURE_LONG_MS, runId: 'free-legacy-design', evalCollector: {},
    ...
    runSkillTest: async (opts: any) => {
      ...
      return { output: timedOut ? '' : mode === 'empty-summary' ? 'Done.' : 'Saved the information architecture and interaction state decisions in plan.md.',
        exitReason: timedOut ? 'timeout' : mode === 'max-turns' ? 'error_max_turns' : 'success',
...
test('legacy Design cleans the fixture when its write fails', async () => {
  ...
  expect(String(result.resultError)).toContain('controlled plan write failure'); expect(result.rows).toEqual([]);
});
