...
export async function captureSectionReads(opts: {
  ...
  const planReviewWritingRule = fullPlanReview
    ? `\n- Native execution window: ${timeout / 1000} seconds. Allocate the first three quarters to required Reads, decisions and section checkpoints; reserve the final ${timeout / 4000} seconds for assembling and verifying completion outputs. Every required section, finding, approval and output still has to be completed; this is pacing guidance, not permission to skip work.
