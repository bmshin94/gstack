...
export async function captureSectionReads(opts: {
  ...
  const planReviewWritingRule = opts.skillName === 'plan-ceo-review'
    ...
    : opts.skillName === 'plan-eng-review'
      ? `\n- Maintain ${outFile} incrementally with Write/Edit. When Scope Challenge finishes, save its outcome. Then, after each completed review section, save that section's complete findings and disposition, including an explicit "No issues found" when applicable. Check the Write/Edit result and Read the saved outcome before advancing to the next section. These checkpoints also apply when no new choice needs approval; do not draft the whole review for one final Write. Preserve original requirements and accepted amendments.\n- Keep the full engineering review in concise, complete records. Preserve all four review sections, every finding and original requirement, required decision fields and comparisons, exact approvals, verification, diagrams, TODOS dispositions, completion summary and GSTACK REVIEW REPORT. Give each independent choice one ID and one authoritative decision record; findings may reference several choice IDs. Keep its complete question and every option before selecting, as the skill requires. Use short summaries and option bullets instead of copying checklists or narrating every check. After a verified record exists, apply answers and amendments with scoped Edit operations; do not regenerate unchanged records or repeat their briefs in the final report. Cross-reference their IDs in the report. Do not write full implementation or test code unless needed to specify an accepted change. At completion, Read the assembled plan, finish missing required outputs with scoped Edits, and perform the skill's full final Read-back gate without rewriting unchanged records. This controls writing only: execute the complete workflow, actually Read every required section, finish every required artifact and verification, then return.`
      : '';
...
${opts.skillName === 'plan-ceo-review' ? `- Save the evolving plan and required review outputs to ${outFile}. Follow each workflow save/readback checkpoint as it occurs; do not defer all persistence to one final Write.` : opts.skillName === 'plan-eng-review' ? `- Save the evolving engineering review and its required report table to ${outFile} at the checkpoints below; completion does not trigger a whole-file rewrite.` : `- When the workflow is complete, write the skill's final output (the full review report / ship plan, including any required report table) to ${outFile}.`}${nativeReviewRule}${planReviewWritingRule}
