...
Rules:
- Compare architectures against the same accepted requirements and declared unchanged contracts. "Minimal viable" is the smallest implementation meeting that baseline; drop no accepted requirement.
- Ask separately about unresolved remedies (error policy, test depth, performance) that can vary without changing the architecture: one issue per AskUserQuestion. Bundle a remedy with an approach only when coupled; explain why they must be chosen together and the exact scope approved. Do not re-ask explicitly accepted requirements or remedies.
