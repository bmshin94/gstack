...
**Order:** Challenge 0A–0C → approve approaches in 0C-bis → select mode in 0F → follow its route below.

Keep a ledger from input reading through Step 0 and outside review. Record behavior, conventions, existing test coverage, and exact approved or pending scope. Reopen a contract or decision only for a concrete contradiction or changed assumption, never speculation or reviewer agreement. Retain actual code risks.
...
Keep each limit in its original unit and retain its dependencies. Changing a limit through reuse or effort savings requires evidence and approval. Finishing faster does not increase the allowed number of deliverables.
...
Before 0F, honor applicable instructions or an accepted approach decision. Cite its authority and resolve 0C-bis without asking again.

Resolve pending decisions one row at a time:

1. **Construct rows.** Record `reference | commitment/contract | current value + approval | proposed value | others fixed/pending`. Each row changes one commitment or value; an issue or helper name is only a reference. For each behavior contract, compare the method and depth of verification, even within one test suite. Carry forward actual approvals; an approach approval covers only its stated scope.
...
Keep explicit user choices and preamble session-kind precedence. If `QUESTION_TUNING: true`, check `plan-ceo-review-mode` via `gstack-question-preference --check` before selecting or presenting a mode. `AUTO_DECIDE` permits the recommended mode; `ASK_NORMALLY` requires a question. These are permissions, not modes. Other interactive selections need a question; `CONDUCTOR_SESSION: true` controls transport only.
...
|------|----------------------|
| SCOPE EXPANSION / SELECTIVE EXPANSION | 0D-prelude → 0D → 0D-POST (including its spec review loop) → 0E |
| HOLD SCOPE | 0D → 0E |
| SCOPE REDUCTION | 0D |
...
1. Run the HOLD SCOPE complexity check below.
2. Run its check for the minimum changes needed.
...
After the opt-in or cherry-pick ceremony, persist the plan below.
