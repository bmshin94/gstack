...
Correct facts. Reuse each actual answer for its authorized work and limits,
without asking about that same choice again.
...
| Question | Route |
|---|---|
| Mode (0E), setup, navigation, document promotion | Use its prescribed menu and preamble transport; record the answer. These do not approve remedies. |
| Scope (0G), TODO, approach, review/spec/outside finding | Run steps 1–4. Prescribed menus replace option labels/count only; keep the full comparison, payload, save, Read-back and answer checks. |
...
- **Descriptions:** each full native `description` contains a 1–2 sentence summary,
  ...
  2 ✅ pros and 1 ❌ con. Keep preamble bullet lengths and its hard-stop exception.
...
3 = shortcut. For different kinds of work, write: "Note: options differ in kind, not coverage — no completeness score."
...
- **Save.** Under the storage policy, save/present the complete current plan,
  ...
  and all exact fields in this layout. Add only absent selectors; do not fence it:
...
literally with the saved fields, except Save-added selector prefixes. Compare
...
**Continue after 0D:**
- Initial pass after 0C: proceed to 0E once required choices are settled, including
  when no new answer was needed.
...
Run all four 0D steps for each unanswered addition or deferral, using the menu
above. Keep other scope fixed or pending; wait for the answer before applying it.
...
- **Blocked:** Return the failed check and complete plan, report and summary.
  Label only unwritten artifacts **not persisted**; a verified report stays
  persisted if its log is forbidden. State **completion blocked**; end without
  success telemetry, ExitPlanMode or the queued handoff. Resume after resolving
  the blocker; no gate pass is claimed.
