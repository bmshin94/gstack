...
Rules:
- Compare architectures against the same accepted requirements and unchanged contracts. "Minimal viable" is the smallest implementation meeting that baseline; drop no accepted requirement.
- Ask separately about unresolved remedies (error policy, test depth, performance) that can vary independently of architecture: one issue per AskUserQuestion. Bundle a remedy with an approach only when coupled; explain that dependency and the exact scope approved. Do not re-ask explicitly accepted requirements or remedies.
...
Present these approach options via AskUserQuestion using the preamble's format with RECOMMENDATION. For alternatives differing in coverage of the same decision, include `Completeness: N/10` on each. Otherwise use `Note: options differ in kind, not coverage — no completeness score.`
