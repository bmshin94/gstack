...
Build one `currentDecision` using the preamble's rules: full question/header and
...
- **Read-back.** Read back: verify every field against `currentDecision` and each citation against
   its source. Inspect both the ledger row and the complete saved `currentDecision`;
   verify their IDs match the question before asking it. For chat, verify the
   complete **not persisted** text. A grid, summary or pointer is insufficient.
