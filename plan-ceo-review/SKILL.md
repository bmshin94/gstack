...
Rules:
- Compare architectural alternatives against the same accepted requirements and declared unchanged contracts. "Minimal viable" means the smallest implementation of that baseline, not silently dropping an accepted requirement.
- Separate independently decidable remedies from the architecture choice. If error policy, test depth, or performance work can vary without changing the architecture, present each unresolved tradeoff separately, one issue per AskUserQuestion. Include a remedy in an approach only when it is coupled to that architecture; explain why they must be chosen together and the exact scope being approved. Do not create extra questions for requirements or remedies already explicitly accepted.
...
Present these approach options via AskUserQuestion using the preamble's AskUserQuestion Format section: include RECOMMENDATION. Include `Completeness: N/10` on every option only when these alternatives differ in coverage of the same decision. If they differ in kind, include: `Note: options differ in kind, not coverage — no completeness score.`
...
First identify the proposed review posture from the modes and defaults below. Keep the approved 0C-bis approach fixed.
...
Then resolve how that posture is selected, using registered ID `plan-ceo-review-mode` and the preamble's session-kind precedence. Retain an explicit user choice. Every other interactive selection requires a question unless the preference check authorizes skipping it. When `QUESTION_TUNING: true`, run the documented `gstack-question-preference --check` before selecting or presenting the mode. `AUTO_DECIDE` authorizes the recommended posture; `ASK_NORMALLY` requires the question. The preference result is permission to skip a question, never a review posture. `CONDUCTOR_SESSION: true` determines question transport.

If an answer is needed, use the preamble's question format and transport: include RECOMMENDATION and `Note: options differ in kind, not coverage — no completeness score.`

**Mode handoff, before 0D:** After the answer arrives or the authorized automatic choice is made, send the user one normal assistant message before any analysis, plan edit, tool call, or next question:
- User-selected or session-selected: `Mode: <selected posture>; approach: <approved 0C-bis approach>.`
...
Fill `<selected posture>` with exactly one of the four named modes above. Keep `(your preference)` unchanged; put rationale in a separate sentence. This direct confirmation completes mode selection; only then continue to 0D. Persisting the choice in the plan does not complete this handoff. Stay in that mode and ask before changing the approved approach.
...
The CEO artifact records scope decisions; it does not replace the full plan. Set
"Plan under review" to the actual plan path, including accepted scope changes. If
the plan exists only in conversation, first save that complete plan to its own
file, distinct from this CEO artifact (never a self-reference). Pass both paths
to the spec reviewer below so it can evaluate the full
requirements without mistaking this scope summary for the entire specification.
...
- "Read both files in full. The CEO document records scope decisions; the source
