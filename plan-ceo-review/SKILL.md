...
- **Read-back.** After the latest successful Write/Edit, Read the ledger row and
   entire saved payload through the last option's full description. Read omitted
   continuations; verify IDs and every field against `currentDecision`, and citations
   against source. This Read is mandatory despite Edit's current-in-context hint;
   even a one-word correction requires a new Read. For chat, verify the complete
   **not persisted** text. A grid, summary or pointer is insufficient.

Stop on failed save/mismatch. A changed decision repeats 0D step 3.

**4. Ask, record the answer, and amend.**
Ask one row per call: copy verified `currentDecision` question, header, labels and
descriptions literally. Do not add a brief, shorten labels or rebuild options;
changes return to step 3's save and Read-back. Only the preamble authorizes
prose/auto-decision transport. A recommendation is not approval.
...
- For each dimension, return PASS or numbered issues with descriptions and suggested fixes. Return overall PASS if all dimensions pass.
...
Report the outcome and the fields below. Show full reviewer output on request. List unresolved issues under "## Reviewer Concerns" in the CEO summary, citing the owning input.
