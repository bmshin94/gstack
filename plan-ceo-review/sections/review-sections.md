...
2. After the actual answer, use a scoped Edit to record that choice and only its authorized amendments, including an explicit deferral if that is what the user chose. Preserve existing plan content and exact earlier approvals; an amendment may carry an accepted constraint forward without claiming that a later section has been reviewed.
