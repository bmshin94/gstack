...
Apply the three test rules in Step 0D: carry approved regression tests forward, separate independently selectable new test additions, and keep tests for undecided behavior pending. Test wording alone does not choose test depth. Parameterizing a lookup and batching a per-order loop remain separate decisions even if one helper implements both; email recovery does not settle instrumentation. Do not invent independent add-ons to fill a third option.
...
1. **Required Outputs:** resolve TODO decisions, assemble tasks and summaries, then write the review report and perform its Read-back.
2. **Saved history:** perform permitted handoff cleanup, write the Review Log, then display the dashboard. Unsaved output stays explicitly not persisted.
3. **Navigation:** choose Next Steps and any docs/designs promotion. Queue the next skill until this review closes. If an answer changes the plan, resolve it through 0D and repeat the affected outputs, report Read-back, log and dashboard in that order.
4. **Terminal gate:** return to the skeleton's Section self-check and EXIT PLAN MODE GATE. Stop on any unmet check; do not record success or run Closing hooks yet.
5. **Closing hooks:** after the gate passes, run the hooks below once, then exit or hand off. No plan or approval changes occur after the terminal gate.
...
# Resolve SLUG and BRANCH in this shell before matching handoff notes.
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)"
...
Return to the skeleton's Section self-check and EXIT PLAN MODE GATE now. Resume
below only after that gate passes; a failed gate must not emit success telemetry.

## Closing hooks

The review and its approvals are complete. Run these hooks once, in order,
...
Run the preamble's **Telemetry (run last)** now, after all skill work above.
The separate nonblocking cache refresh follows telemetry:
...
Now finish with ExitPlanMode where the host requires it, or return to the calling
workflow. Then perform the chosen next-skill handoff; do not reopen this review
