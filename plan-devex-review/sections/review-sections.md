...
declared baseline. Keep one working list of evidence, exact approved decisions,
unresolved gaps and required verification. A missing detail in a summary is an
unknown, not proof the existing product lacks it. Reopen a contract or decision
only by identifying concrete contradictory evidence or a changed assumption.
...
Define what each proposed measurement includes before choosing it. Developer
TTHW runs from the selected persona encountering the documented start through
their first understood, useful result, including reading, setup and first-run
state. Record observed human onboarding separately from automated execution
time; a warm snippet timer is neither a fresh-start check nor a human benchmark.
Keep estimates labeled until measured. Prefer the simplest method that measures
the chosen target. A target tier does not itself approve telemetry, an automated
