...
After startup, follow the preparation sections below through Confidence Calibration. Read Decision procedure as the rule for later choices. Start the review at Scope Challenge, then complete Sections 1–4 in order.
...
Inspect recent review/refactor/revert commits for the selected paths. Record
recurring issues or reversals and check those paths during this review.
...
Voice and late changes. Finish one choice before starting another. Initial scope
selectors have their own recording rules. The write policy governs persistence;
the preamble governs question tools and authorized auto-decisions.
...
Select one pending ID and build `currentDecision`:
...
`Note: options differ in kind, not coverage — no completeness score.` Bundling
policies earns no completeness; test stars rate existing tests, not decisions or findings.
...
and pending choices. Put all deliberation in the native question/descriptions;
a saved-only Pros/cons block cannot supply missing decision context.
...
and all options. Use exactly one A–D record selector for each option: keep an
existing selector or prefix one. Compare the label separately from that notation.

When revising, replace the whole current payload. Keep other choices' headings,
content and approvals intact; move superseded payloads to History. Do not leave
duplicate Question, Header or Options fields.
...
rules. Setup and Scope Challenge selectors follow this dispatch order with their
own recording rules.
...
6. **Finish.** Run Learning hooks, then return to the entrypoint's Section
   ...
   mode. Only after both pass, run success telemetry and cache refresh; call
   ExitPlanMode only in host plan mode.
