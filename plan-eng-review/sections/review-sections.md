...
what the selected code does now and how each proposed remedy would change it. Give
...
user/host limits, including active-plan-only; one path authorizes no other. Edit
implementation only with explicit authority. Never write forbidden paths or
silently replace a requested destination.
...
Keep behavior with necessary code/tests/docs. Compare alternative mechanisms for
that same fixed behavior in one question. Give each independently selectable
runtime outcome its own ID.
...
Before saving, check each option's full label, description and grid column
against every row. They must agree on all commitments and conditions; a label
cannot override its description or grid:
...
1. **Check decisions.** Reuse the current Approval readiness result. Derive the
   unresolved count from each record's current `State`, actual answer and accepted scope.
