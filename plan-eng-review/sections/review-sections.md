...
Use only the options offered. Show unchanged values and `pending` explicitly. Check each value against Step 1: a shared new value still needs approval, even when it does not vary between options. Required implementation and proof of one approved contract stay common work, not extra commitments.
...
Copy the current and offered-option values from Step 3 into `Option comparisons`, including the approved values held fixed, common required work and other pending choices. Include the source or reviewer behind the finding. Mark undecided rows `pending`; do not add their remedies as accepted work.
