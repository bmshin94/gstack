...
Resolve pending choices with this sequential loop. Setup and Scope Challenge selectors use the same dispatch order with their own recording rules. Follow the preamble's Tool resolution and failure fallback, including authorized auto-decisions.

1. **Ask and wait:** For a tool question, set `currentDecision` to the single question object for the audited brief: one `D<N>` and alternative answers for that one choice. Send the audited brief without substantive additions using `AskUserQuestion({ questions: [currentDecision] })`: one question for one choice per AskUserQuestion call. The array contains exactly one question object; other pending IDs wait for later calls. An obvious fix still needs an answer unless exact prior approval covers it. **STOP for each pending decision.** Wait for its answer before applying that remedy, moving to the next section or calling ExitPlanMode. Do not queue another call while its answer is pending.
2. **Apply the answer:** Record the actual option, answer reference and accepted scope separately from draft options. Apply only those amendments to the working plan with a scoped Edit before taking the next choice; present them in read-only mode. Update its decision record and leave others unchanged.
3. **Refresh the next choice:** Use the updated plan and actual answer to rebuild the next brief; for remedies, return through **Frame the choices**, **Compare one choice**, and the save step. Hold the chosen value fixed and ask the next pending choice if still relevant; record why an irrelevant choice needs no question. Advance to the next section only when this section has no pending choices.
