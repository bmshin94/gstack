...
These are engineering instincts, not extra checklist items. Apply them throughout the review.
...
Apply boring defaults to architecture, systems over heroes to tests, Brooks to complexity, and innovation budgets to new infrastructure.
...
If complexity reaches 8+ files or 2+ new classes/services, STOP before section work. Use the preamble's question flow in this order:

1. Name what's overbuilt. If the smaller design requires removing a feature, ask about that cut separately first.
2. Compare a smaller component structure with the original structure. Every option must include, drop or defer the same features, following earlier approvals. Leave unresolved feature choices undecided. Preserve established contracts in both designs. Keep approved security, error-handling, test and performance fixes unchanged; unresolved fixes stay undecided.
3. Ask the user to choose the component design. That answer does not approve, drop or postpone any independent fix; ask about each fix separately before making that change.

**STOP.** Until the user resolves this choice, an earlier approval covers it, or an authorized auto-decision resolves it, do not enter Section 1 or call ExitPlanMode. Keep Step 0 findings, revised approaches and proposed fixes out of all plan files until then, including new files and findings appendices. You may copy the user's original plan unchanged, without adding review content.
