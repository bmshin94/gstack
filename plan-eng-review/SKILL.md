...
If the complexity check triggers (8+ files or 2+ new classes/services), STOP before section work. Use the preamble's question flow: name what's overbuilt, propose a minimal version meeting the goal, ask: reduce or proceed as-is. Keep independent security, error, test and performance remedies constant or pending in every scope option. Fewer components does not approve a pending remedy; ask about each separately before applying it.
