...
Treat each independently selectable remedy as its own finding even when an outside voice
groups several under one topic. Split those remedies into separate AskUserQuestion calls;
keep implementation details and tests that directly establish one chosen contract together.
Scope reduction does not approve other independent remedies; ask separately.
