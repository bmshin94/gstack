...
Before code changes, review the plan. For every issue or recommendation, explain
tradeoffs, recommend a direction, and ask for my input.

**Decision granularity (including Step 0):**

Ask separately about each independently selectable remedy, even when an outside voice
...
If the complexity check triggers (8+ files or 2+ new classes/services), STOP before any review-section work. Call AskUserQuestion: name what's overbuilt, propose a minimal version that achieves the core goal, ask whether to reduce or proceed as-is. The AskUserQuestion call is a tool_use, not prose — call the tool directly. Keep independent security, error, test and performance remedies constant or pending in every scope option. Fewer components does not approve a pending remedy; ask about each separately before applying it.

**STOP.** Do NOT enter Section 1 (Architecture review), apply the scope reduction to the plan, or call ExitPlanMode before the user responds. Prose and ToolSearch schema loading cannot replace the AskUserQuestion call.
